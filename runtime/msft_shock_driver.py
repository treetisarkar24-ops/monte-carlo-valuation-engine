"""
Chunked driver for the MSFT shock stage.

The shocked convergence sweep exceeds the 45s per-call wall limit available in
this environment. This driver reproduces case_study_runner.stage_shock EXACTLY,
but injects a disk-memoizing wrapper around run_monte_carlo_with_shocks through
the convergence module's existing `runner` seam. Because the memo keys on
(n_simulations, random_seed) and the underlying engine is seeded, every cached
list is bit-identical to what the real runner would have produced. No engine,
convergence, grid, or shock-calibration logic is modified.

Run it repeatedly: each call resumes from cache, makes progress on the sweep,
and exits at a time budget. Once every batch is cached, the final call completes
the sweep + production + channel diagnostics and writes msft_results.json.
"""
...
def memo_runner(base, cfg):
    ...
    if time.time() - START > BUDGET:
        # Out of time for this call; stop so the parent process exits before
        # being reaped. Already-computed batches are safely on disk.
...
def main():
    ...
    # ---- shocked convergence sweep (memoized, resumable) ----
    try:
    ...
    z = shock.z_star
    # production run at z** (single run; cache it too for determinism)
    res = memo_runner(base, MCConfig(n_simulations=z, random_seed=SEED))
    arr = np.asarray(res)

    # ---- channel diagnostics (5000-sim apparatus, matches stage_shock) ----
